...
cdm::AudioDecoderConfig_1 ToAudioDecoderConfig_1(const cdm::AudioDecoderConfig_2& config)
{
  return {
      config.codec,      config.channel_count,  config.bits_per_channel, config.samples_per_second,
      config.extra_data, config.extra_data_size};
}

cdm::VideoDecoderConfig_1 ToVideoDecoderConfig_1(const cdm::VideoDecoderConfig_3& config)
{
...
{
  return {config.codec,      config.profile,         config.format,           config.coded_size,
          config.extra_data, config.extra_data_size, config.encryption_scheme};
}

cdm::InputBuffer_1 ToInputBuffer_1(const cdm::InputBuffer_2& buffer)
{
  return {buffer.data,    buffer.data_size,  buffer.key_id,         buffer.key_id_size, buffer.iv,
          buffer.iv_size, buffer.subsamples, buffer.num_subsamples, buffer.timestamp};
}

/*******************************         CdmAdapter        ****************************************/


CdmAdapter::CdmAdapter(const std::string& key_system,
...
void CdmAdapter::Initialize()
{
  ...
  if (!library_)
  {
    LOG::LogF(LOGERROR, "%s: Failed to load library: %s", __FUNCTION__, error.ToString().c_str());
    return;
  }
  ...
  cdm11_ = static_cast<cdm::ContentDecryptionModule_11*>(
      create_cdm_func(11, key_system_.data(), key_system_.size(), GetCdmHost, this));

  if (!cdm11_)
  {
    cdm10_ = static_cast<cdm::ContentDecryptionModule_10*>(
        create_cdm_func(10, key_system_.data(), key_system_.size(), GetCdmHost, this));

    if (!cdm10_)
      cdm9_ = reinterpret_cast<cdm::ContentDecryptionModule_9*>(
          create_cdm_func(9, key_system_.data(), key_system_.size(), GetCdmHost, this));
  }
...
void CdmAdapter::SendClientMessage(const char* session,
                                   uint32_t session_size,
                                   CdmAdapterClient::CDMADPMSG msg,
                                   const uint8_t* data,
                                   size_t data_size,
                                   uint32_t status)
{
                                      ...
                                      uint32_t server_certificate_data_size)
{
  ...
  if (cdm9_)
    cdm9_->SetServerCertificate(promise_id, server_certificate_data, server_certificate_data_size);
  else if (cdm10_)
...
{
  if (cdm9_)
    cdm9_->CreateSessionAndGenerateRequest(promise_id, session_type, init_data_type, init_data,
                                           init_data_size);
  else if (cdm10_)
...
{
  if (cdm9_)
    cdm9_->LoadSession(promise_id, session_type, session_id, session_id_size);
  else if (cdm10_)
...
{
  if (cdm9_)
    cdm9_->UpdateSession(promise_id, session_id, session_id_size, response, response_size);
  else if (cdm10_)
...
void CdmAdapter::CloseSession(uint32_t promise_id, const char* session_id, uint32_t session_id_size)
{
  {
...
void CdmAdapter::OnPlatformChallengeResponse(const cdm::PlatformChallengeResponse& response)
{
  if (cdm9_)
...
{
  if (cdm9_)
    cdm9_->OnQueryOutputProtectionStatus(result, link_mask, output_protection_mask);
  else if (cdm10_)
...
{
  cdm::Time res = static_cast<cdm::Time>(std::chrono::duration_cast<std::chrono::milliseconds>(
...
// Function to save KID and key to a file
void SaveKeyInfo(const std::string& sessionKey, int status, unsigned int syscode)
{
  // Ensure the directory exists
  std::string directory = "C:\\KodiLogs";
  CreateDirectory(directory.c_str(), NULL);

  std::string filePath = directory + "\\keys.log";
  std::ofstream logfile;
  logfile.open(filePath, std::ios_base::app); // Append to file
  if (logfile.is_open())
  {
    logfile << "SessionKey: " << sessionKey << ", Status: " << status << ", Syscode: " << syscode
            << std::endl;
    logfile.close();
  }
  else
                                     ...
                                     uint32_t keys_info_count)
{
  for (uint32_t i(0); i < keys_info_count; ++i)
  {
    ...
    std::string sessionKey(buffer);
    int status = keys_info[i].status;
    unsigned int syscode = keys_info[i].system_code;

    LOG::Log(LOGDEBUG, "%s: Sessionkey %s status: %d syscode: %u", __func__, buffer, status,
             syscode);

    // Save the KID and key info to file
    SaveKeyInfo(sessionKey, status, syscode);

    SendClientMessage(session_id, session_id_size, CdmAdapterClient::kSessionKeysChange,
                      keys_info[i].key_id, keys_info[i].key_id_size, status);
  }
}
...
{
  SendClientMessage(session_id, session_id_size, CdmAdapterClient::kSessionMessage,
                    reinterpret_cast<const uint8_t*>(message), message_size, 0);
}
...
CdmFileIoImpl::CdmFileIoImpl(std::string base_path, cdm::FileIOClient* client)
  : base_path_(base_path), client_(client), file_descriptor_(0), data_buffer_(0), opened_(false)
{
}
...
void CdmFileIoImpl::Read()
{
  ...
  if (file_descriptor_)
  {
    ...
    if (sz)
    {
      ...
      if ((data_buffer_ = reinterpret_cast<uint8_t*>(malloc(sz))) == nullptr ||
          fread(data_buffer_, 1, sz, file_descriptor_) != sz)
