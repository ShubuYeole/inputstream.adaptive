...
public:
  ...
  void CloseSession(uint32_t promise_id, const char* session_id, uint32_t session_id_size);

  void RemoveSession(uint32_t promise_id, const char* session_id, uint32_t session_id_size);
  ...
  cdm::Status InitializeAudioDecoder(const cdm::AudioDecoderConfig_2& audio_decoder_config);

  cdm::Status InitializeVideoDecoder(const cdm::VideoDecoderConfig_3& video_decoder_config);
  ...
  void OnPlatformChallengeResponse(const cdm::PlatformChallengeResponse& response);
  ...
  // Declare the SaveKeyInfo function
...
private:
  ...
  void Initialize();
  void SendClientMessage(const char* session,
                         uint32_t session_size,
